...
function AgentWorkspace({ importedGuides, publishedGuides, onSignOut }: { importedGuides: Guide[]; publishedGuides: Guide[]; onSignOut: () => void }) {
  ...
  return <main className="agent-app">
    ...
    <div className="agent-page">
      ...
      {view === "home" && area === "products" && <ProductHome activeProduct={activeProduct} onChooseProduct={chooseProduct} onChooseCategory={chooseCategory} query={query} setQuery={setQuery} onSearch={search} onLiveSearch={(value) => { setQuery(value); if (value.trim()) setView("search"); }} />}
...
function ProductHome({ activeProduct, onChooseProduct, onChooseCategory, query, setQuery, onSearch, onLiveSearch }: { activeProduct: typeof products[number]; onChooseProduct: (productId: string) => void; onChooseCategory: (categoryId: string) => void; query: string; setQuery: (value: string) => void; onSearch: (event: FormEvent<HTMLFormElement>) => void; onLiveSearch: (value: string) => void }) {
  return <>
    <section className="product-hero"><div><span className="eyebrow light"><Sparkles size={13} /> Live Chat Knowledge Base</span><h1>Pilih produk, lalu pilih kendalanya.</h1><p>Pedoman disusun mengikuti produk dan kategori agar kamu dapat merespons pelanggan lebih cepat.</p><form className="product-search" onSubmit={onSearch}><Search size={18} /><input value={query} onChange={(event) => { setQuery(event.target.value); onLiveSearch(event.target.value); }} placeholder="Ketik kendala pelanggan untuk mencari..." /><button type="submit" aria-label="Cari"><ArrowRight size={15} /></button></form></div><div className="hero-guide-card"><BookOpen size={21} /><strong>1 kondisi</strong><span>1 pedoman utuh</span><small>Tier 1 dan eskalasi ada dalam satu flow</small></div></section>
