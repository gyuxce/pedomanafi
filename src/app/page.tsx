...
function LoginScreen({ onSignIn, authError, authBusy }: { onSignIn: (email: string, password: string) => Promise<void>; authError: string; authBusy: boolean }) {
  ...
  return <><main className="login-page">
    <section className="login-shell">
      <div className="login-brand"><span className="brand-mark large"><BookOpen size={22} /></span><div><strong>ATLAS</strong><span>Agent Triage &amp; Live-chat Assistance System</span></div></div>
...
function ConditionSummary({ value }: { value: string }) {
  ...
  return <div className="condition-summary">{visiblePoints.map((point, index) => <p key={`${point}-${index}`}>{points.length > 1 && <span>{String(index + 1).padStart(2, "0")}</span>}{point}</p>)}{points.length > visiblePoints.length && <small>+{points.length - visiblePoints.length} poin lainnya</small>}</div>;
}
