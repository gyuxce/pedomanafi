...
function AgentWorkspace({ importedGuides, publishedGuides, onSignOut }: { importedGuides: Guide[]; publishedGuides: Guide[]; onSignOut: () => void }) {
  ...
  return <main className="agent-app">
    <header className="agent-topbar">
      <div className="agent-topbar-inner"><div className="agent-brand"><span className="brand-mark"><BookOpen size={17} /></span><strong>AFI</strong><span>Knowledge</span></div><nav className={`agent-primary-nav ${mobileMenu ? "is-open" : ""}`} aria-label="Navigasi knowledge"><button className={area === "products" ? "active" : ""} onClick={() => chooseArea("products")}>Produk</button><button className={area === "operational" ? "active" : ""} onClick={() => chooseArea("operational")}>Pedoman Operasional</button></nav><div className="agent-top-actions"><div className="global-search"><Search size={16} /><input value={query} onChange={(event) => { const value = event.target.value; setQuery(value); setView(value.trim() ? "search" : "home"); }} onFocus={() => { if (query.trim()) setView("search"); }} placeholder="Cari pedoman..." aria-label="Cari pedoman" /></div><button className="top-avatar" aria-label="Profil Agent">AD</button><button className="icon-button mobile-menu-trigger" aria-label="Buka menu" onClick={() => setMobileMenu(!mobileMenu)}>{mobileMenu ? <X size={19} /> : <Menu size={19} />}</button><button className="icon-button signout-button" aria-label="Keluar" onClick={onSignOut}><LogOut size={17} /></button></div></div>
    </header>
...
function ProductHome({ activeProduct, onChooseProduct, onChooseCategory }: { activeProduct: typeof products[number]; onChooseProduct: (productId: string) => void; onChooseCategory: (categoryId: string) => void }) {
  return <>
    <section className="product-hero"><div><span className="eyebrow light">Live Chat Knowledge Base</span><h1>Pilih produk, lalu pilih kendalanya.</h1><p>Pedoman disusun mengikuti produk dan kategori agar kamu dapat merespons pelanggan lebih cepat.</p><p className="product-hero-note">Gunakan pencarian global di header untuk menemukan pedoman dari semua produk.</p></div><div className="hero-guide-card"><BookOpen size={21} /><strong>1 kondisi</strong><span>1 pedoman utuh</span><small>Tier 1 dan eskalasi ada dalam satu flow</small></div></section>
